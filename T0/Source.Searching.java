        ...
        Searching()
        {
            try {
                BufferedReader br = new BufferedReader (new InputStreamReader(System.in));
            		N = Integer.parseInt(br.readLine());//membaca banyak data yg akan di baca
                X = new long[N+1];
                X[0] = 0;
                	for (int i = 1; i <= N; i++)
            		      X[i] =  X[i-1]+Integer.parseInt(br.readLine());//membaca sambil mengisi kedalam array

              		int M = Integer.parseInt(br.readLine());//jumlah pertanyaan
